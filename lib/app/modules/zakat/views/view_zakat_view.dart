...
class ViewZakatView extends StatefulWidget {
  final ZakatModel zakat;

  const ViewZakatView({super.key, required this.zakat});

  @override
  State<ViewZakatView> createState() => _ViewZakatViewState();
}

class _ViewZakatViewState extends State<ViewZakatView> {
  double _zakat = 0;
  double _nisab = 0;
  double _asset = 0;
  double _gold = 0;
  ...
  void initState() {
    setState(() {
      _gold = double.tryParse("${widget.zakat.gold}") ?? 0;
      _silver = double.tryParse("${widget.zakat.silver}") ?? 0;
      _cashOnHand = double.tryParse("${widget.zakat.cashOnHand}") ?? 0;
      _cashInBank = double.tryParse("${widget.zakat.cashInBank}") ?? 0;
      _receivable = double.tryParse("${widget.zakat.receivable}") ?? 0;
      _payable = double.tryParse("${widget.zakat.payable}") ?? 0;
      _investment = double.tryParse("${widget.zakat.investment}") ?? 0;
      _stock = double.tryParse("${widget.zakat.stock}") ?? 0;
      _nisab = double.tryParse("${widget.zakat.nisab}") ?? 0;
      _asset = double.tryParse("${widget.zakat.asset}") ?? 0;
      _zakat = double.tryParse("${widget.zakat.zakat}") ?? 0;
    });
    super.initState();
  }
                ...
                onPressed: () async {
                  try {
                    if (_formKey.currentState!.saveAndValidate()) {
                      ...
                      ZakatModel? zakat = await ZakatProvider.update(
                        widget.zakat.uuid!,
                        data,
                      );
