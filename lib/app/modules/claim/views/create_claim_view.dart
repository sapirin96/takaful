...
import 'package:takaful/app/data/models/claim_model.dart';
...
class CreateClaimView extends StatefulWidget {
  const CreateClaimView({super.key});

  @override
  State<CreateClaimView> createState() => _CreateClaimViewState();
}

class _CreateClaimViewState extends State<CreateClaimView>
    with AutomaticKeepAliveClientMixin<CreateClaimView> {
                ...
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                      ...
                      Obx(
                        () => ConditionalBuilder(
                          condition: Get.find<AuthService>()
                                  .user
                                  .value
                                  ?.tokenableType
                                  ?.toLowerCase() !=
                ...
                onPressed: () async {
                  try {
                    if (_formKey.currentState!.saveAndValidate()) {
                      ...
                      var memberId = Get.find<AuthService>()
                                  .user
                                  .value
                                  ?.tokenableType
                                  ?.toLowerCase() ==
                          ...
                          : _formKey.currentState?.value['member_id'];
                      ...
                      /// create claim
                      ClaimModel? claim = await ClaimProvider.store(
                        data,
                      );

                      /// check if claim is created
                      if (claim?.uuid != null) {
  ...
  Future<void> _getCompensation() async {
    try {
      ...
      var memberId =
          Get.find<AuthService>().user.value?.tokenableType?.toLowerCase() ==
                  'member'
              ? Get.find<AuthService>().user.value?.id
              : _formKey.currentState?.value['member_id'];
