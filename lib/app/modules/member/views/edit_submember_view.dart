...
class EditSubmemberView extends StatefulWidget {
  final SubmemberModel submember;

  const EditSubmemberView({super.key, required this.submember});

  @override
  State<EditSubmemberView> createState() => _EditSubmemberViewState();
}

class _EditSubmemberViewState extends State<EditSubmemberView>
    with AutomaticKeepAliveClientMixin<EditSubmemberView> {
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: Container(
            ...
            child: FormBuilder(
              key: _formKey,
              initialValue: widget.submember.toJson(),
                ...
                child: ListView(
                  children: [
                    ...
                    FormBuilderImagePicker(
                      ...
                      placeholderImage: widget.submember.profilePictureUrl !=
                              null
                          ? NetworkImage(widget.submember.profilePictureUrl!)
                          : null,
                    ),
                    Row(
                    ...
                    FormBuilderDateTimePicker(
                      initialValue: widget.submember.dateOfBirth != null
                          ? DateTime.parse(widget.submember.dateOfBirth!)
                          : null,
                      name: 'date_of_birth',
                    ...
                    FormBuilderDateTimePicker(
                      initialValue: widget.submember.jointDate != null
                          ? DateTime.parse(widget.submember.jointDate!)
                          : null,
                      name: 'joint_date',
                      inputType: InputType.date,
                      decoration: InputDecoration(
                      ...
                      children: [
                        Expanded(
                          child: FormBuilderImagePicker(
                            ...
                            galleryLabel: Text('Gallery'.tr),
                            placeholderImage:
                                widget.submember.frontDocumentUrl != null
                                    ? NetworkImage(
                                        widget.submember.frontDocumentUrl!)
                                    : null,
                          ),
                        ),
                        const SizedBox(width: 10),
                        Expanded(
                          child: FormBuilderImagePicker(
                            ...
                            galleryLabel: Text('Gallery'.tr),
                            placeholderImage:
                                widget.submember.backDocumentUrl != null
                                    ? NetworkImage(
                                        widget.submember.backDocumentUrl!)
                                    : null,
                          ),
                        ),
                      ],
                ...
                onPressed: () async {
                  try {
                    if (_formKey.currentState!.saveAndValidate()) {
                      ...
                      /// Convert back document to multipart
                      ...
                      /// Convert front document to multipart
                      ...
                      /// Update submember
                      SubmemberModel? submember =
                          await SubmemberProvider.update(
                        widget.submember.uuid!,
                        data,
                      );

                      /// Update submember success
                      if (submember?.uuid != null) {
                        Get.back(result: true);
                      }
                    }
                  } catch (error) {
                    /// Update submember failed
